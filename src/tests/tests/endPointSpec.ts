...
describe('testing suite for server', () : void => { // testing / endpoint for server to see if server at '/' connected
    ...
    it('testing "/resize" functionality if we get an image back. \n Response type should be "image/jpeg"', async () : Promise < void > => {
        const response = await request.get('/resize?filename=matteo-badini&width=1000&height=500');
    ...
    it('testing "/resize" end point to fail with bad input. \n We should get text (error message) instead of image as response type', async () : Promise < void > => {
        const response = await request.get('/resize?filename=matteobadini&width=1000&height=500');
